...
class Results(object):
  ...
  def WriteToFile(self, file_name):
    # Find out how many iterations were run for each benchmark. We ASSUME these
    # are all the same.
    iterations = len(self.traces[0]['results'])
    with open(file_name, "w") as f:
      writer = csv.writer(f)
      header = ['process_exec_num', 'bench_name'] + range(iterations)
      writer.writerow(header)
      for bench in self.traces:
        assert len(bench['results']) == iterations, \
                 '%s has %i iterations, when %i were expected' % \
                   (bench['graphs'][1], len(bench['results']), iterations)
        row = [0, bench['graphs'][1]] + bench['results']
        writer.writerow(row)
...
def Main(args):
  ...
  parser.add_option("--extra-flags",
                    ...
                    default="")
  parser.add_option("--csv-test-results",
                    help=("Path to a file for storing CSV results, for use with"
                          " the warmup_stats script."))
  parser.add_option("--csv-test-results-no-patch",
                    help="Path to a file for storing CSV results from run "
                         "without patch.")
  ...
  if (options.csv_test_results_no_patch and
  ...
  if options.csv_test_results:
    options.csv_test_results = os.path.abspath(options.csv_test_results)

  if options.csv_test_results_no_patch:
    options.csv_test_results_no_patch = os.path.abspath(
        options.csv_test_results_no_patch)
  ...
  with CustomMachineConfiguration(governor = options.cpu_governor,
    ...
    if options.csv_test_results:
      results.WriteToFile(options.csv_test_results)
    else:  # pragma: no cover
      print results

  if options.csv_test_results_no_patch:
    results_no_patch.WriteToFile(options.csv_test_results_no_patch)
  else:  # pragma: no cover
